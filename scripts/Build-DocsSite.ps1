...
function Get-DocumentCategory {
    ...
    if ($value -match 'reference|terraform') {
        return [pscustomobject]@{
            Key = "reference"
            Title = "Operations & Reference"
            Description = "Supporting reference material, configuration details, and implementation boundaries."
            Icon = "⌁"
        }
    }
    if ($value -match 'runbook|deployment|validation') {
        return [pscustomobject]@{
            Key = "runbooks"
            Title = "Deployment Runbooks"
            Description = "Procedures for initializing, planning, applying, and validating the landing zone."
            Icon = "▦"
        }
    }
    return [pscustomobject]@{
        ...
        Description = "Understand the landing-zone topology, shared controls, and design boundaries."
        Icon = "◇"
    }
}
...
function Render-Page {
    ...
    $page = $page.Replace('{{HEADER_PATH}}', (Encode-Html $(if ($IsHome) { 'Documentation portal' } else { $PageTitle })))
...
$categoryDefinitions = @(
    [pscustomobject]@{ Key = "guides"; Title = "Architecture & Design"; Description = "Understand the landing-zone topology, shared controls, and design boundaries."; Icon = "◇" },
    [pscustomobject]@{ Key = "runbooks"; Title = "Deployment Runbooks"; Description = "Procedures for initializing, planning, applying, and validating the landing zone."; Icon = "▦" },
    [pscustomobject]@{ Key = "reference"; Title = "Operations & Reference"; Description = "Supporting reference material, configuration details, and implementation boundaries."; Icon = "⌁" }
)
...
<main class="catalog-view">
  ...
  <section class="category-overview">
    <div class="category-intro">
      ...
      <p>Choose a collection to browse architecture guidance, deployment runbooks, or supporting reference material.</p>
    </div>
...
"@
Write-Utf8File (Join-Path $outputRoot "index.html") (Render-Page $indexContent "Documentation portal" "index.html" $true)
