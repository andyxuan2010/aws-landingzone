...
The GitHub Pages workflow treats the Markdown in `docs/` as the source of truth. It generates a multi-page static site in `_site/`, including `index.html`, document pages, navigation, and search metadata, then deploys only that generated directory. Build and validate it locally with:

```powershell
./scripts/Build-DocsSite.ps1
./scripts/Test-DocsSite.ps1
```
